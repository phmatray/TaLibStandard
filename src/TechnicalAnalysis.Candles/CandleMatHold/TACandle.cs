...
public static partial class TACandle
{
    ...
    /// <returns></returns>
    public static CandleIndicatorResult CdlMatHold<T>(
        int startIdx, int endIdx, T[] open, T[] high, T[] low, T[] close, T penetration)
        where T : IFloatingPoint<T>
    {
        return new CandleMatHold<T>(open, high, low, close)
    ...
    public static CandleIndicatorResult CdlMatHold<T>(
        ...
        where T : IFloatingPoint<T>
    {
        return CdlMatHold(startIdx, endIdx, open, high, low, close, T.CreateChecked(0.5));
    }
}
