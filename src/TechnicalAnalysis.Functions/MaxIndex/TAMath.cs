...
public static partial class TAMath
{
    ...
    /// in the data series.
    /// </remarks>
    public static MaxIndexResult MaxIndex(int startIdx, int endIdx, double[] real, int timePeriod = 30)
    {
        int outBegIdx = 0;
        int outNBElement = 0;
    ...
    /// <param name="endIdx">The ending index for the calculation range.</param>
    /// <param name="real">Input array of real values.</param>
    /// <param name="timePeriod">The number of periods to look back for finding the maximum value (default: 30).</param>
    /// <returns>A MaxIndexResult containing the indices of maximum values over each rolling window.</returns>
    /// <remarks>
    ...
    /// </remarks>
    public static MaxIndexResult MaxIndex(int startIdx, int endIdx, float[] real, int timePeriod = 30)
        => TAMathHelper.Execute(startIdx, endIdx, real, (s, e, r) => MaxIndex(s, e, r, timePeriod));
}
