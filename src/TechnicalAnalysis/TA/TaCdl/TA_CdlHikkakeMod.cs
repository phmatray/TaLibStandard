...
namespace TechnicalAnalysis
{
    internal static partial class TACore
    {
                ...
                bool patternRecognition =
                    ...
                    (
                        (
                            ...
                            inLow[i] < inLow[i - 1] &&
                            // (bull) 2nd: close near the low
                            inClose[i - 2] <= inLow[i - 2] + GetCandleAverage(Near, nearPeriodTotal, i - 2, inOpen, inHigh, inLow, inClose)
                        )
                        ||
                        (
                            // (bear) 4th: higher high and higher low
                            ...
                            inLow[i] > inLow[i - 1] &&
                            // (bull) 2nd: close near the top
                            inClose[i - 2] >= inHigh[i - 2] - GetCandleAverage(Near, nearPeriodTotal, i - 2, inOpen, inHigh, inLow, inClose)
        ...
        {
            return Math.Max(1, GetCandleAvgPeriod(Near)) + 5;
        }
    }
}
